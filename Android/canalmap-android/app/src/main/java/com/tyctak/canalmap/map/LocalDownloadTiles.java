...
public class LocalDownloadTiles {
    ...
    public LocalDownloadTiles(final MapView mapView) {
        mTileProvider = mapView.getTileProvider();
        mTileWriter = mapView.getTileProvider().getTileWriter();
        mMapView = mapView;
    }

    public LocalDownloadTiles(final MapView mapView, IFilesystemCache writer) {
        mTileProvider = mapView.getTileProvider();
        mTileWriter = writer;
        mMapView = mapView;
    }
     ...
     */
    public boolean loadTile(final OnlineTileSourceBase tileSource, final MapTile tile, Integer keyValue, String waterwayGuid) {
...
//        }
        ...
        try {
            ...
            Global.getInstance().getDb().writeTileRequest(tileURLString, keyValue, waterwayGuid);
...
//Log.d(Constants.APP_TAG, "New Tile lat " + tile.x + " lon " + tile.y);
     ...
     * @since
     */
    public DownloadingTask downloadAreaAsyncNoUI(Context ctx, ArrayList<GeoPoint> geoPoints, final int zoomMin, final int zoomMax, final CacheManagerCallback callback, String waterwayGuid) {
        DownloadingTask task = new DownloadingTask(ctx, geoPoints, zoomMin, zoomMax, callback, false);
        task.execute(waterwayGuid);
        mPendingTasks.add(task);
        return task;
    }
    ...
    public class DownloadingTask extends CacheManagerTask {
        ...
        protected Integer doInBackground(Object... params) {
            String waterwayGuid = (String)params[0];
            int errors = downloadArea(waterwayGuid);
            return errors;
        }
         ...
         */
        public int downloadArea(String waterwayGuid) {
                ...
                for (int zoomLevel = mZoomMin; zoomLevel <= mZoomMax; zoomLevel++) {
                    ...
                    for (int y = mUpperLeft.y; y <= mLowerRight.y; y++) {
                        for (int x = mUpperLeft.x; x <= mLowerRight.x; x++) {
                            ...
                            Integer keyValue = ((zoomLevel << zoomLevel) + tileX << zoomLevel) + tileY;;

                            boolean ok = loadTile(tileSource, tile, keyValue, waterwayGuid);
                                        ...
                                        for (int xAround = tile.x + ofsx; xAround <= tile.x + 1 + ofsx; xAround++) {
                                            for (int yAround = tile.y + ofsy; yAround <= tile.y + 1 + ofsy; yAround++) {
                                                ...
                                                if (!foundTilePoint) {
                                                    ...
                                                    int keyValue = ((zoomLevel << zoomLevel) + tileX << zoomLevel) + tileY;
                                                    //Drawable currentMapTile = mTileProvider.getMapTile(tile);


                                                    boolean ok = loadTile(tileSource, tileToDownload, keyValue, waterwayGuid);
                                                    if (!ok) {
                                                        errors++;
                                                    }
                                                    tileCounter++;
